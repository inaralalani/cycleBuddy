...
public class TextApp {
	...
	private void enterPersonInfo() {
		...
		boolean alcoholBoolean = false;
		if (alcoholConsumption.equalsIgnoreCase("yes")) {
			alcoholBoolean = true;
		}
	...
	private void runVisualizer() {
		...
		Visualization.printCalendar(aPerson, Calendar.getInstance(), Visualization.periodDays);
		if (Visualization.nextMonthPeriodDays.size() > 1) 
			Visualization.printCalendar(aPerson, Visualization.getNextMonthCalendar(Calendar.getInstance()), Visualization.nextMonthPeriodDays);
